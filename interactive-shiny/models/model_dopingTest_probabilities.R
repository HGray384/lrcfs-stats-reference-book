...
output$display_dopingTest_priorOddsCalcs = renderUI({
...
output$display_dopingTest_likelihoodRatioInfo = renderUI({
...
output$display_dopingTest_likelihoodRatioCalcs = renderUI({
  
  likelihoodRatio = formatNumberForDisplay(likelihoodRatio())
  
  formulas = c(paste0("\\text{Likelihood Ratio} = \\frac{\\text{Sensitivity}}{1-\\text{Specificity}} &= \\frac{",input$doping_prob_Sensitivity,"}{1-",input$doping_prob_Specificity,"}"))
  formulas = c(formulas, paste0("&= \\frac{",input$doping_prob_Sensitivity,"}{",1-input$doping_prob_Specificity,"}"))
...
output$display_dopingTest_posteriorOddsInfo = renderUI({
...
output$display_dopingTest_posteriorOddsCalcs = renderUI({
  
  posteriorOdds= formatNumberForDisplay(posteriorOdds())
  
  formulas = c(paste0("\\text{Posterior Odds} &= \\text{Prior Odds} * \\text{Likelihood Ratio}"))
  formulas = c(formulas, paste0("&= ",formatNumberForDisplay(priorOdds),"*",formatNumberForDisplay(likelihoodRatio())))
