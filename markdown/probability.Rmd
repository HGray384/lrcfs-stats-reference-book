...
The double coin toss example above was extended to a game in which player 1 won if two heads were tossed and player 2 won if two tails were tossed. They drew if there was any combination of head and tail. When thinking about the probabilities of each player winning, we conditioned on coin 1 showing a head and also on coin 1 showing a tail. In each conditioning, one of the players' probabilities of winning reduced to 0 and the other's increased to 0.5.

Suppose the game is altered slightly so that player 1 now tosses the coins and does not show player 2 which sides were facing up. As part of the background information in this example, assume that player 1 always tells the truth and is tossing the coins in such a way to guarantee 0.5 probability of a head for either coin. After tossing the coins, player 1 only tells player 2 whether player 2 has won or not, i.e. whether double tails were tossed or not. If player 2 does not win, then they are given another chance to win by guessing the result of coin 1's toss. If they guess correctly, they win this new game. What should player 2 guess?

Begin by describing the uncertainty for player 2:

- **object**: outcome of coin 1 after learning that both coins were not tails,
- **source**: lack of knowledge about the outcome of coin 1 (player 1 knows),
- **type**: epistemic,
- **level**: direct,
- **magnitude**: this is what we need to find.

This is a classic situation of epistemic uncertainty. Before the coin tosses, there is aleatory uncertainty - there is an unavoidable randomness to the future outcomes of these coin tosses. Focussing only on the uncertainty of coin 1, both a head and a tail for coin 1 have equal probability of 0.5. These are prior odds of evens, or 1:1.

After the coin tosses, there are two possible situations.

1. Player 2 is told that they have won, in which case there is no uncertainty for them any longer because they know that coin 1 must have resulted in tails.

2. Player 2 is told that they did not win, and now they have epistemic uncertainty about the results of both coin tosses. Player 2 is now offered the opportunity to guess coin 1 to win. This is direct uncertainty about the toss of coin 1 and can be easily quantified. 

The outcomes of the double coin toss are presented as an expected frequency tree in Figure \@ref(fig:freq-tree-double-coin-bet-2) below.
