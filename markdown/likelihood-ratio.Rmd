...
It is easier to see the connection between multiple pieces of evidence and competing propositions by using a graphical model. A graphical model is a visual representation of the evidence and case circumstances. This approach is becoming more popular amongst some scientific groups as more methodology is being developed, e.g. @koeijer2020. An example of a simple graphical model with multiple pieces of evidence is shown below:

```{r serial-evidence-chain-diagram, echo=FALSE, fig.cap="An example of a serial evidence chain diagram. Nodes on the diagram depend upon previous nodes and this dependence should be accounted for in probability and LR calculations.", out.width = '100%', fig.align = 'center'}
...
parallel evidence chain
```{r parallel-evidence-chain-diagram, echo=FALSE, fig.cap="An example of a parallel evidence chain diagram. Item 1 and 2 from the chain of evidence only depend on each other through the suspect and so could be considered as independent conditional on the suspect having committed the activity, $H_p$.", out.width='100%', fig.align='center'}
