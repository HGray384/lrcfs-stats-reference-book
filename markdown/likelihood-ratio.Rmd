...
Statistics are the subject of constant scrutiny in daily life, be it from experts or otherwise. Since they are used to inform very important decisions and policies (social, medical or otherwise), subjecting them to scrutiny allows .

The LR is also open to the same type of scrutiny. Two main reasons for this are 1) it is formed using empirical observations where possible, which can change over time amongst other variables and 2) whether an opinion which leads to creating an LR is a widely-held belief within a field of expertise or not, an expert's opinion is always subjective. 

The robustness of an LR is only as good as the robustness of the probabilities from which it is formed. Any critical assessment of an LR is only a critical assessment of its underlying probabilities. It is not an assessment of the inherent properties that taking a ratio of two probabilities might give rise to.

Assessing the robustness of an LR is aided by making a distinction between types of information that the expert uses when constructing conditional probabilities.
