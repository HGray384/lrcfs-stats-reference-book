...
How well does this sample mean represent the actual number of publications each year in our sample? We can answer that by calculating the **sample standard deviation**. To show some intuition for this, we can plot the number of publications each year against the sample mean to highlight the difference.
...
[Roy: it would be great to make the above interactive, so the red lines pivoted round to be on the same scale and showed them being squared and then averaged and square rooted to calculate the sample standard deviation.]
...
We've gone through the calculation of the sample statistics for the number of GSR-related publications in this dataset, how useful are they?
