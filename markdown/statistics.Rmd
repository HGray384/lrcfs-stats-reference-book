...
Population 2: burglaries in the Greater London area of England,
...
Sample 3 is also a convenience sample, this time of reported footwear. The degree to which the submitted footwear marks are representative of all footwear in a local area (e.g. population 3) depends upon the representativeness of the volunteers who submit them. If a certain age group of people is over-represented in the volunteers then the sample will be **biased** in favour of that age group. If desired, statistical methods can be applied to take this bias into account, e.g. by weighing the over-represented age group's submission proportionally less compared to other age groups. This is one way polling companies attempt to make a potentially biased sample more representative.

Sampling adds another layer of uncertainty to consider, which lends itself to working with probability. In any sampling method, there is uncertainty about how representative the sample is of the population from which it is drawn, and also a natural randomness as to which members are sampled. This represents both epistemic and aleatory uncertainty. The epistemic uncertainty arises because we do not know some characteristics of the population (which is why we are sampling from it). Learning more about the population to be sampled from reduces this uncertainty, and so the sampling procedure itself can reduce this.

The aleatory uncertainty is because there will always be randomness when sampling. The randomness can be due to a random sampling procedure, but even within convenience samples there is a randomness as to which convenient members are sampled. **Sample size** is an important factor for characterising the aleatory uncertainty associated with sampling. The more we perform a sampling procedure in a population, the more we learn about its variability. 

But sample size isn't everything: a small but carefully constructed random sample can be more representative of the population than a large but careless convenience sample.

Increasing the sample size only reduces the uncertainty to a point, 

and so often an optimal sample size can be specified before any sample is collected. 
